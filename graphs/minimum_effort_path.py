...
def min_effort_path(heights):
    ...
    maximum_effort = [[float("inf") for _ in range(len(heights[0]))] for _ in range(
        len(heights))]     
    ...
    while pq != []:
        ...
        for i in range(4):          
            ...
            if new_row >= 0 and new_row < n_num_of_rows and new_col >= 0 and new_col < n_num_of_cols:
                ...
                if diff < maximum_effort[new_row][new_col]:
                    maximum_effort[new_row][new_col] = diff
                    heapq.heappush(pq, (diff, new_row, new_col))

    return (maximum_effort[n_num_of_rows-1][n_num_of_cols - 1])
